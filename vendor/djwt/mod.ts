...
export function decode<PayloadType extends Payload | unknown = unknown>(
  ...
  try {
    const arr = jwt
      .split(".")
      .map(base64url.decodeBase64Url)
      .map((uint8Array, index) =>
        index === 0 || index === 1
          ? JSON.parse(decoder.decode(uint8Array as AllowSharedBufferSource))
          : uint8Array
      );
 ...
 *       (base64url as any)(UTF8(JWS Protected Header)) || '.' ||
 *       (base64url as any)(JWS Payload) || '.' ||
 *       (base64url as any)(JWS Signature)
 */
function createSigningInput(header: Header, payload: Payload): string {
  return `${base64url.encodeBase64Url(encoder.encode(JSON.stringify(header)))}.${
    base64url.encodeBase64Url(encoder.encode(JSON.stringify(payload)))
  }`;
}
